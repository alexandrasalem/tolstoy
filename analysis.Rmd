...
data <- bind_rows(childhood, boyhood, anna_karenina, resurrection)
...
```
```{r}
labels <- c(anna_karenina = "Anna Karenina, 1877", boyhood = "Boyhood, 1854", childhood = "Childhood, 1852", resurrection = "Resurrection, 1899")

ggplot(data_final, aes(index, sentiment, color = fct_relevel(title, "boyhood", "childhood", "anna_karenina", "resurrection"))) +
  geom_point(size = .5, show.legend = FALSE) +
  geom_hline(aes(yintercept=.5))+
  geom_segment(aes(x=index, xend = index, y = sentiment, yend = .5)) +
  scale_color_discrete(name = "Book", labels = labels) +
  facet_wrap(~fct_relevel(title, "boyhood", "childhood", "anna_karenina", "resurrection"), ncol = 2, scales = "free_x", labeller=labeller(labels)) +
